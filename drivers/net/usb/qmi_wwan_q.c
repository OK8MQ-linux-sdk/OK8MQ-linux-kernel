...
static struct sk_buff *qmap_qmi_wwan_tx_fixup(struct usbnet *dev, struct sk_buff *skb, gfp_t flags) {
	...
	if (unlikely(pQmapDev == NULL)) {
		goto drop_skb;
	} else if (likely(pQmapDev->qmap_mode && !pQmapDev->link_state)) {
			  ...
			  const struct usb_device_id *prod)
{
	...
	if (!status) {
		...
		if (pQmapDev->qmap_mode) {
			...
			else //for these modules, if send pakcet before qmi_start_network, or cause host PC crash, or cause modules crash
...
MODULE_LICENSE("GPL");
#define QUECTEL_WWAN_VERSION "Quectel_Linux&Android_QMI_WWAN_Driver_V1.0.1"
